...
import { mergeAvailableActors, KNOWN_JOB_SOURCES, orderApifyPicker } from '@/lib/job-sources/sources';
import { orderFreeSources } from '@/lib/job-sources/free';
import { summarizeSearchOutcome, type SearchSummary } from '@/lib/jobs/search-summary';
import { COUNTRY_OPTIONS, normalizeCountryCode, sourceCoverageGroupTitle, sourceCoverageLabel, sourceCoveragePriority, sourceCoversCountry } from '@/lib/job-sources/countries';
...
const DEFAULT_PRESETS: SearchPreset[] = [
  ...
  {
    id: 'preset-backoffice-berlin',
    name: 'Backoffice & Data',
    title: 'Sachbearbeiter Datenmanagement',
    country: 'DE',
    location: 'Berlin',
    remote: 'any',
    limit: 10,
...
export function SearchModal({ isOpen, onClose, onSearchComplete }: SearchModalProps) {
  const { t } = useI18n();
  ...
  const [selectedFree, setSelectedFree] = useState<string[]>(['arbeitsagentur']);
  ...
  const [loadingPhase, setLoadingPhase] = useState<'searching' | 'triaging'>('searching');
  ...
  useEffect(() => {
    ...
    const defaultsForCountry = (code: string) => {
      const normalized = normalizeCountryCode(code);
      return {
        actors: orderApifyPicker(KNOWN_JOB_SOURCES, normalized)
          .filter((source) => source.defaultEnabled && sourceCoversCountry(source.coverage, normalized))
          .slice(0, 2)
          .map((source) => source.actorId),
        free: orderFreeSources(normalized)
          .filter((source) => source.id !== 'ats' && !source.needsKey && sourceCoversCountry(source.coverage, normalized))
          .slice(0, normalized === 'DE' ? 3 : 2)
    ...
    Promise.all([
      ...
      .then(([data, profileData]) => {
        if (!isMounted) return;
        const savedCountry = normalizeCountryCode(profileData.profile?.country || 'DE');
        ...
        try {
          ...
          if (lastSearchRaw) {
            const lastSearch = JSON.parse(lastSearchRaw);
            activeCountry = normalizeCountryCode(lastSearch.country || savedCountry);
          }
        } catch {}
        setCountry(activeCountry);

        const saved: string[] = data.apify?.actorIds?.length ? data.apify.actorIds : [];
        const actors = mergeAvailableActors(saved, KNOWN_JOB_SOURCES);
        const countryDefaults = defaultsForCountry(activeCountry);
        setAvailableActors(actors);
        setSelectedActors((current) => {
          const stillAvailable = current.filter((actor) => actors.includes(actor));
          if (stillAvailable.length > 0) return stillAvailable;
          const savedRecommended = saved.filter((actor) => actors.includes(actor));
          return savedRecommended.length > 0 ? savedRecommended.slice(0, 1) : countryDefaults.actors;
        });
        ...
        if (Array.isArray(data.free?.enabledSources)) {
          const enabled = data.free.enabledSources.filter((id: string) =>
            orderFreeSources(activeCountry).some((source) =>
              source.id === id &&
              (source.id !== 'ats' || hasCompanyBoards) &&
              sourceCoversCountry(source.coverage, activeCountry)
            )
          );
          setSelectedFree(enabled.length > 0 ? enabled : countryDefaults.free);
        } else {
          setSelectedFree(countryDefaults.free);
        }
  ...
  const knownActorIds = Object.values(KNOWN_JOB_SOURCES).map((source) => source.actorId);
  const customActorIds = availableActors.filter((actor) => !knownActorIds.includes(actor));
  ...
  const chooseCountry = (value: string) => {
    ...
    setSelectedActors((current) => {
      const compatible = current.filter((actor) => {
        const known = Object.values(KNOWN_JOB_SOURCES).find((source) => source.actorId === actor);
        return !known || sourceCoversCountry(known.coverage, nextCountry);
      });
      if (compatible.length > 0) return compatible;
      return orderApifyPicker(KNOWN_JOB_SOURCES, nextCountry)
        .filter((source) => source.defaultEnabled && sourceCoversCountry(source.coverage, nextCountry))
        .slice(0, 2)
        .map((source) => source.actorId);
    });
    setSelectedFree((current) => {
      const compatible = current.filter((id) => {
        const source = orderFreeSources(nextCountry).find((item) => item.id === id);
        return source && sourceCoversCountry(source.coverage, nextCountry);
      });
      if (compatible.length > 0) return compatible;
      return orderFreeSources(nextCountry)
        .filter((source) => !source.needsKey && sourceCoversCountry(source.coverage, nextCountry))
        .slice(0, nextCountry === 'DE' ? 3 : 2)
        .map((source) => source.id);
    });
  };
      ...
      <DialogContent className="sm:max-w-2xl rounded-2xl p-5 sm:p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 pt-1">
          <div className="space-y-1.5">
            <Label htmlFor="search-title" className="text-xs font-medium text-neutral-700">
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3">
            <div className="space-y-1.5 flex-1 min-w-40">
              <Label htmlFor="search-country" className="text-xs font-medium text-neutral-700">
                {t('search.country')}
              </Label>
              <select
                id="search-country"
            ...
            <div className="space-y-1.5 flex-1 min-w-40">
              <Label htmlFor="search-loc" className="text-xs font-medium text-neutral-700">
                {t('search.location')}
              </Label>
              <Input
                id="search-loc"
            ...
            <div className="space-y-1.5 flex-1 min-w-40">
              <Label htmlFor="search-remote" className="text-xs font-medium text-neutral-700">
                {t('search.workplace')}
              </Label>
              <select
                id="search-remote"
            ...
            <div className="space-y-1.5 flex-1 min-w-40">
              <Label htmlFor="search-posted" className="text-xs font-medium text-neutral-700">
                {t('search.posted')}
              </Label>
              <select
                id="search-posted"
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="space-y-1.5 flex-1 min-w-0">
                <Label className="text-xs font-medium text-neutral-700">
                  {t('search.apify')} <span className="font-normal text-neutral-400">({t('search.apifyHint')})</span>
                </Label>
                <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto pr-1">
                  {orderApifyPicker(KNOWN_JOB_SOURCES, country).map((source) => {
                    const isSelected = selectedActors.includes(source.actorId);
                    const shortName = source.name.replace(/\s*\(.*\)\s*$/, '');
              ...
              <div className="space-y-1.5 w-full sm:w-28 shrink-0">
                <Label htmlFor="search-limit" className="text-xs font-medium text-neutral-700">
                  {t('search.limitEach')}
                </Label>
                <Input
                  id="search-limit"
            ...
            <div className="space-y-2">
              <Label className="text-xs font-medium text-neutral-700">
                {t('search.freeSources')} <span className="font-normal text-neutral-400">({t('search.noApifyCost')})</span>
              </Label>
              {[0, 1, 2, 3].map((priority) => {
                const sources = orderFreeSources(country).filter((source) => sourceCoveragePriority(source.coverage, country) === priority);
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          className="size-3.5 rounded border-neutral-300 cursor-pointer text-blue-600"
                        />
                        <span className="font-mono">{actor.split('/')[1] || actor}</span>
                      </label>
                    );
                  })}
                </div>
            ...
            <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-4 flex flex-col gap-2.5">
              ...
              <ul className="flex flex-col gap-1">
                {summary.lines.map((line) => (
                  <li key={line.source} className="flex flex-wrap items-baseline justify-between gap-2 text-xs">
                    <span className="font-medium text-neutral-700">{line.source}</span>
                    <span className="text-neutral-500 tabular-nums">
                      {line.discovered} found · {line.newJobs} new · {line.duplicates} dupes
                      {line.dismissed > 0 ? ` · ${line.dismissed} dismissed` : ''}
                      {line.excluded > 0 ? ` · ${line.excluded} blocked` : ''}
                    </span>
                  </li>
                ))}
                {summary.failures.map((failure) => (
                  <li key={failure.source} className="flex flex-wrap items-baseline justify-between gap-2 text-xs">
                    <span className="font-medium text-neutral-700">{failure.source}</span>
                    <span className="text-red-600">{failure.error.slice(0, 120)}</span>
                  </li>
                ))}
              </ul>
          {summary.allQuiet && (
                <p className="text-xs text-neutral-500 leading-relaxed">
                  {summary.dismissed > 0 || summary.excluded > 0
                    ...
                    : ''}
                  International boards rarely carry German-titled roles — try English keywords for them.
                </p>
              )}
          ...
          <div className="flex items-center justify-between pt-2 border-t border-neutral-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                type="submit"
                size="sm"
                disabled={loading || (selectedActors.length === 0 && selectedFree.length === 0)}
                title={
              ...
              >
                ...
                <span>{loading ? (loadingPhase === 'triaging' ? t('search.triaging') : t('search.finding')) : t('discover.findRoles')}</span>
              </Button>
            </div>
          </div>
        </form>
